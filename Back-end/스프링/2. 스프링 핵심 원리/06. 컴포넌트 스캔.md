...
`@Target(ElementType.TYPE)`: *TYPE* → Class 레벨에 붙는 애노테이션
...
**컴포넌트 스캔 대상에 추가할 클래스 - @MyIncludeComponent 추가**
...
**컴포넌트 스캔 대상에서 제외할 클래스 - @MyExcludeComponent 추가**
...
- `includeFilters` 에 `@MyIncludeComponent` 을 추가해서 IncludeBean이 스프링 빈에 등록된다.
- `excludeFilters` 에 `@MyExcludeComponent` 을 추가해서 ExcludeBean은 스프링 빈에 등록되지 않는다.

### FilterType 옵션

FilterType 옵션은 총 5가지가 있다.
...
예를 들어, IncludeBean도 빼고 싶으면 다음과 같이 추가하면 된다.
...
@ComponentScan(
        ...
        excludeFilters = {
                ...
                @Filter(type = FilterType.ASSIGNABLE_TYPE, classes = IncludeBean.class)
        }
)
static class ComponentFilterAppConfig {
}
...
```java
Overriding bean definition for bean 'memoryMemberRepository' with 
a different definition: replacing
```
...
`application.properties`에 `spring.main.allow-bean-definition-overriding=true`를 추가하면 된다.
